...
impl ColliderFactory for CollisionShape {
    fn collider_builder(&self) -> ColliderBuilder {
        match self {
            ...
            CollisionShape::TriMesh { positions, indices } => {
                trimesh_builder(positions.clone(), indices.clone())
            }
        }
    }
}
...
mod tests {
    ...
    fn build_trimesh() {
        ...
        let builder_1 = CollisionShape::TriMesh {
            positions: positions.clone(),
            indices: indices.clone(),
        }
        .build(Entity::new(0), RigidBody::default(), None, None);

        let builder_2 = CollisionShape::TriMesh {
            positions: positions
                .clone()
                .iter()
        ...
        .build(Entity::new(1), RigidBody::default(), None, None);
